  ...
  <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-red-900/50 border border-red-500/50 rounded-xl p-6 max-w-md text-center shadow-lg shadow-red-900/30">
      ...
      <p className="text-red-300 text-sm sm:text-base">
        The <code>API_KEY</code> environment variable is not set. Please ensure
        it is configured correctly to use the AI features of this studio.
      </p>
    </div>
  </div>
)
...
export default function App() {
  ...
  const tourSteps: TourStep[] = [
    {
      ...
      content:
        "This quick tour will walk you through the key features. Let's get started!",
    },
    {
      selector: '#persona-selector',
      title: 'Select a Persona',
      content:
        'Choose an AI assistant or the Image Generator from this menu. Each persona has a unique personality and purpose.',
    },
    {
      selector: isImageMode ? '#image-prompt-input' : '#chat-input',
      title: isImageMode ? 'Describe Your Image' : 'Chat with the AI',
      content: isImageMode
        ? 'Type a detailed description of the image you want to create here.'
        : 'Type your message here to start a conversation with the selected AI persona.',
    },
    {
      selector: '#command-palette-hint',
      title: 'Command Palette',
      content:
        'Press Ctrl+K (or Cmd+K) anywhere to quickly switch personas, clear your chat, and more.',
    },
  ]
  ...
  const commandActions: CommandAction[] = [
    ...PERSONAS.map((p) => ({
      ...
      section: 'Personas',
      icon: (
        <span className="text-xs font-bold w-full h-full flex items-center justify-center bg-white/10 rounded-sm text-gray-300">
          {p.name
            .split(' ')
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-start bg-[#0b0b0f] text-gray-200 p-4 sm:p-6 md:p-8 font-sans">
      ...
      <div aria-hidden className="pointer-events-none fixed inset-0 -z-10">
        <div className="absolute -top-40 left-1/2 -translate-x-1/2 h-80 w-[36rem] bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.05),transparent_60%)] blur-2xl" />
        <div className="absolute -bottom-40 left-1/3 h-80 w-[36rem] bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.04),transparent_60%)] blur-2xl" />
      </div>
      ...
      <div className="relative w-full max-w-3xl flex flex-col items-center p-[1.5px] sm:p-[2px] rounded-2xl">
        ...
        <motion.div
          className="hidden md:block absolute -inset-1 rounded-2xl bg-[conic-gradient(from_90deg_at_50%_50%,#ffffff_0%,#4a5568_50%,#ffffff_100%)] opacity-20"
        ...
        <div className="relative w-full rounded-2xl border border-white/10 bg-neutral-900/70 backdrop-blur-xl shadow-2xl shadow-black/40 p-4 sm:p-6 md:p-7">
          ...
          <div
            ...
            className="w-full text-center text-[11px] sm:text-xs text-gray-500 mb-3 sm:mb-4 -mt-1"
          >
            Pro-tip: Press{' '}
            <kbd className="px-1.5 py-0.5 border border-white/20 bg-white/10 rounded">
              Ctrl+K
            </kbd>{' '}
      ...
      />
      {isTourActive && (
        <OnboardingTour steps={tourSteps} onComplete={handleTourComplete} />
      )}
    </div>
  )
}
