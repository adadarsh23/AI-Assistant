...
/**
 * Creates and attaches a custom header to a <pre> element for code blocks.
 * The header includes the language name and a copy-to-clipboard button.
 */
const addCodeBlockHeader = (preElement: HTMLPreElement) => {
  // Prevent adding multiple headers on re-renders
  if (preElement.querySelector('.code-block-header')) {
    return
  }
  ...
  useEffect(() => {
    if (contentRef.current) {
      ...
      preElements.forEach((preElement) => {
        // The header logic is now self-contained.
        addCodeBlockHeader(preElement)

        // This part wraps the code for scrolling, which is still needed.
        if (preElement.querySelector('.code-wrapper')) return
        const code = preElement.querySelector('code')
        if (!code) return
